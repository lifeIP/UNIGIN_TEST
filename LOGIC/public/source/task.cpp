...
namespace triangle_sort {
	...
	static int number_of_recursions = 0;
	static int max_number_of_recursions = 100;
	static const int The_Number_of_Elements_Processed_by_the_Thread = 5000;//The number on which the number of threads running simultaneously depends. (The higher this number, the fewer threads will be started)
	class Sort
	{
	public:
		/// <summary>
		...
		iterators_list static partition(transmitted_iterators iterator, std::function<bool(const Triangle& a)>func);
		...
		int static sortVectorInAscendingOrder(transmitted_iterators iterator);
		...
		bool static finding_and_recording_triangle_matches(transmitted_iterators match_search_iterators,
			std::vector<int>::iterator out_count_begin, std::vector<int>::iterator out_count_end);

	};
};
triangle_sort::iterators_list triangle_sort::Sort::partition(transmitted_iterators iterator, std::function<bool(const Triangle& a)>func) {
...
int triangle_sort::Sort::sortVectorInAscendingOrder(transmitted_iterators iterator) {
	...
	auto iterators_of_separated_vectors = partition(iterator, [&](const Triangle& A)->bool
		{
			...
			auto get_the_point_as_far_away_from_the_origin_as_possible = [&](const Triangle& A) {
				...
				if (std::sqrt(pow(A.a.x, 2) + std::pow(A.a.y, 2)) - std::sqrt(pow(A.b.x, 2) + pow(A.b.y, 2)) > 0) {
					if (std::sqrt(A.a.x * A.a.x + A.a.y * A.a.y) - std::sqrt(A.c.x * A.c.x + A.c.y * A.c.y) > 0) {
						max = A.a;
					}
					else max = A.c;
				}
				else {
					if (std::sqrt(A.b.x * A.b.x + A.b.y * A.b.y) - std::sqrt(A.c.x * A.c.x + A.c.y * A.c.y) > 0) {
						max = A.b;
					}
					else max = A.c;
				}
			...
			return std::sqrt(max.x * max.x + max.y * max.y) - std::sqrt(max_second.x * max_second.x + max_second.y * max_second.y) < 0;
		});
	...
	if (number_of_recursions <= max_number_of_recursions && size > The_Number_of_Elements_Processed_by_the_Thread) {
	...
	else if (number_of_recursions <= max_number_of_recursions) {
		sortVectorInAscendingOrder(first_part_of_the_truncated_vector);
		sortVectorInAscendingOrder(second_part_of_the_truncated_vector);
	}
	return 0;
}
bool triangle_sort::Sort::finding_and_recording_triangle_matches(transmitted_iterators match_search_iterators,
	std::vector<int>::iterator out_count_begin, std::vector<int>::iterator out_count_end) {
	...
	auto comparison_of_triangles = [&](const Triangle& A, const Triangle& B)->int {
		...
		float a = sqrt(pow(_A.x - _B.x, 2) + pow(_A.y - _B.y, 2));
		if (a <= 0.1)
		{
		...
		else if (a <= 1.5)
		{
			if ((int)A.a.x == (int)B.a.x)
			{
				if ((int)A.a.y == (int)B.a.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.a.x == (int)B.b.x) {
				if ((int)A.a.y == (int)B.b.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.a.x == (int)B.c.x) {
				if ((int)A.a.y == (int)B.c.y)
					return 1; //The points have relative equality.
			}

			if ((int)A.b.x == (int)B.a.x)
			{
				if ((int)A.b.y == (int)B.a.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.b.x == (int)B.b.x) {
				if ((int)A.b.y == (int)B.b.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.b.x == (int)B.c.x) {
				if ((int)A.b.y == (int)B.c.y)
					return 1; //The points have relative equality.
			}

			if ((int)A.c.x == (int)B.a.x)
			{
				if ((int)A.c.y == (int)B.a.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.c.x == (int)B.b.x) {
				if ((int)A.c.y == (int)B.b.y)
					return 1; //The points have relative equality.
			}
			if ((int)A.c.x == (int)B.c.x) {
	...
	auto search_for_matches = [&](const int& begin, const int& end)
	{
		for (int i = begin; i < end; ++i) {
			for (int j = begin; j < end - 1; ++j) {
				int answer = comparison_of_triangles(*(match_search_iterators.begin + i), *(match_search_iterators.begin + j + 1));
				if (answer == 1 || answer == 0) {
					++* (out_count_begin + *(match_search_iterators.begin_counter + i));
					//++* (out_count_begin + *(match_search_iterators.begin_counter + j + 1));
				}
			}
		}
	};
...
void Task::checkIntersections(const std::vector<Triangle>& in_triangles, std::vector<int>& out_count)
{
	...
	triangle_sort::transmitted_iterators iterator_trnsmt;
	...
	triangle_sort::Sort::sortVectorInAscendingOrder(iterator_trnsmt);
	
	triangle_sort::Sort::finding_and_recording_triangle_matches(iterator_trnsmt, out_count.begin(), out_count.end());
}